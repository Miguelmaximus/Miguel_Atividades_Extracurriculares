class AtividadeExtracurricular {
  constructor(nome, descricao, participantes) {
    ...
    this.participantes = Array.isArray(participantes) ? participantes : [];
  }
  ...
  set descricao(novaDescricao) {
    if (typeof novaDescricao === "string" && novaDescricao.length > 10) {
...
function excluirAtividade(index) {
  const nome = atividades[index].nome;
...
function editarAtividade(index) {
  ...
  if (novaDescricao && novaDescricao.trim().length > 10) {
    atividade.descricao = novaDescricao.trim();
  } else {
    alert("Descrição inválida, mantendo antiga.");
  }
  ...
  if (novosParticipantes) {
    atividade.participantes = novosParticipantes
      .split(',')
      .map(p => p.trim())
      .filter(p => p.length > 0);
  }
